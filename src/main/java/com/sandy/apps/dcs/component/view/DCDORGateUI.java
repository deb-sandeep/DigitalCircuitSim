...
public class DCDORGateUI extends DCDGateUI implements Serializable {

    private DCDORGate andGate ;

    public DCDORGateUI(DCDORGate gate) {
    ...
    public void paint(Graphics2D g) {
        ...
        g.drawArc(tlx - 6, tly + 5, 20, height - 10, -90, 180) ;

        g.drawLine(tlx + 6, tly + 5, tlx + 19, tly + 5) ;
        g.drawLine(tlx + 6, tly + height - 5, tlx + 19, tly + height - 5) ;

        g.drawLine(tlx + 20, tly + 5, tlx + 30, tly + height / 2 - 1) ;
        g.drawLine(tlx + 21, tly + height - 5, tlx + 30, tly + height / 2) ;
